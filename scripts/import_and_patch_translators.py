...
def comment_initial_json(text: str) -> (str, bool):
...
def append_exports(text: str) -> (str, bool):
...
def process_file(path: Path) -> (bool, bool):
    changed = False
    commented = False
    appended = False
...
def patch_all():
    ...
    for f in js_files:
        ...
        try:
            c, a = process_file(f)
            if c: commented_count += 1
            if a: appended_count += 1
        except Exception as e:
            print('Error processing', f, e)

    print(f'Processed {total} files: commented {commented_count}, appended exports {appended_count}')
