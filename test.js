...
function urlToCacheName(url) {
  ...
  const b = Buffer.from(String(url || ""), "utf8")
    .toString("base64")
    .replace(/\+/g, "-")
    .replace(/\//g, "_")
    .replace(/=+$/, "");
  ...
  for (let i = 0; i < testCases.length; i++) {
    ...
    try {
      ...
      if (!html) {
        ...
        try {
          await writeCachedHtml(url, html);
        } catch (e) {}
      }
      ...
      try {
        delete global.__test_current_url;
      } catch (e) {}
      if (!out) {
      ...
      if (
        e &&
        e.message &&
        e.message.includes("multi-select not supported") &&
        tc.items === "multiple"
      ) {
