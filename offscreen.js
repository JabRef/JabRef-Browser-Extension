...
try {
  ...
  if (token) {
    ...
    try {
      ...
      port.onMessage.addListener((message) => {
        console.log(
          "[Offscreen] Received port message:",
          ...
          runTranslator(message.payload)
            .then((result) => {
              try {
                ...
                console.error("[Offscreen] port.postMessage failed", e);
              }
            })
            .catch((error) => {
              try {
  ...
  console.warn("[Offscreen] Failed to parse URL token", e);
}
...
try {
  ...
  __fetchPort.onDisconnect.addListener(() => {
    console.warn("[Offscreen] fetch port disconnected");
    __fetchPort = null;
  });
} catch (e) {
    ...
    runTranslator(event.data.payload)
      .then((result) => {
        try {
          event.source.postMessage(
            { type: "OFFSCREEN_RESPONSE", response: { success: true, result } },
            "*",
          );
        } catch (e) {
          console.error(
            "[Offscreen] Failed posting OFFSCREEN_RESPONSE to window",
            e,
          );
        }
      })
      .catch((error) => {
        try {
          ...
          console.error(
            "[Offscreen] Failed posting OFFSCREEN_RESPONSE error to window",
            e,
          );
        }
      });
...
async function runTranslator({ translatorPath, htmlString, url }) {
  ...
  // Create ZU helper
  // Use the shared ZU implementation from the shared shims to avoid duplication.
  // createZU accepts the document and an options object; pass the page URL as base.
  const { createZU } = await import("./sources/zoteroShims.js");
  const ZU = createZU(doc, { baseUrl: url });

  // Use `cleanAuthor` provided by the shared ZU shim (available on `ZU` returned from createZU).
  // The previous local implementation was removed to avoid duplication.

  // Create Zotero shim (shared implementation)
  const { createZoteroShim } = await import("./sources/zoteroShims.js");
  ...
  Zotero.Item = function (itemType) {
    ...
    const logFetchProxy = (absolute, response) => {
      if (chrome.runtime.lastError) {
        console.error(
          "[Offscreen] runtime.lastError during fetch proxy:",
          chrome.runtime.lastError.message,
        );
      } else if (!response || !response.ok) {
        console.error(
          "[Offscreen] background fetch failed:",
          absolute,
          response,
        );
      }
    ...
    if (type === "import") {
      ...
      return {
        ...
        setHandler: (name, handler) => {
          if (name === "itemDone") itemHandler = handler;
        },
        translate: async () => {
  ...
  const requestText = async (u, opts) => {
    ...
    if (chrome && chrome.runtime && chrome.runtime.sendMessage) {
      ...
      if (__fetchPort) {
        return await new Promise((resolve, reject) => {
          ...
          try {
            ...
            console.warn(
              "[Offscreen] fetch port postMessage failed, falling back to sendMessage:",
              e && e.message,
            );
          ...
          console.warn(
            "[Offscreen] port-based fetch failed, err:",
            err && err.message,
          );
        });
      }
      ...
      return await new Promise((resolve, reject) => {
        try {
          console.log(
            "[Offscreen] proxying fetch to background (sendMessage):",
            absolute,
          );
          chrome.runtime.sendMessage(
            { action: "fetch", url: absolute },
            (response) => {
              if (chrome.runtime.lastError) {
                console.warn(
                  "[Offscreen] runtime.lastError during fetch proxy:",
                ...
                if (
                  lastErrMsg &&
                  lastErrMsg.includes("Receiving end does not exist")
                ) {
                  console.log(
                    "[Offscreen] Falling back to direct fetch due to missing receiver:",
                    absolute,
                  );
                  fetch(absolute, opts)
                    .then(async (r) => {
                    ...
                    .catch((fe) => {
                      console.error(
                        "[Offscreen] Direct fetch fallback failed:",
                        fe && fe.message,
                      );
                      return reject(
                        new Error(
                          lastErrMsg || (fe && fe.message) || "Fetch failed",
                        ),
                      );
                    });
                  return;
                ...
                return reject(
                  new Error(lastErrMsg || "No response from runtime"),
                );
              }
              if (!response || !response.ok) {
                ...
                return reject(
                  new Error(
                    response?.error || `HTTP ${response?.status || "error"}`,
                  ),
                );
              }
          ...
          console.error("[Offscreen] exception proxying fetch", e);
          reject(e);
        }
      });
    }
  ...
  await new Promise((resolve, reject) => {
    ...
    script.onerror = (e) => {
      reject(new Error("Failed to load translator script from extension URL"));
    };
  ...
  console.log(
    "[Offscreen] doWeb completed, checking _lastItem:",
    Zotero._lastItem,
  );
