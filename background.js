...
chrome.runtime.onConnect.addListener((port) => {
  try {
    if (port && port.name && port.name.startsWith('offscreen_')) {
      ...
      port.onDisconnect.addListener(() => { offscreenPorts.delete(port.name); console.log('[Background] Offscreen port disconnected:', port.name); });
    }
  } catch (e) { console.warn('[Background] onConnect handler error', e); }
});
    ...
    if (port && port.name === 'fetch') {
      ...
      port.onMessage.addListener(async (msg) => {
        ...
        try {
            ...
            try {
              port.postMessage({ type: 'fetch_result', id, ok: resp.ok, status: resp.status, text });
            } catch (pmErr) {
              console.warn('[Background] Unable to postMessage to fetch port (disconnected):', pmErr && pmErr.message);
            }
          } catch (err) {
            console.error('[Background] fetch(port) error:', url, err && err.message);
            try {
              port.postMessage({ type: 'fetch_result', id, ok: false, error: err && err.message });
            } catch (pmErr) {
              console.warn('[Background] Unable to postMessage error to fetch port (disconnected):', pmErr && pmErr.message);
            }
          }
      });
      port.onDisconnect.addListener(() => { console.log('[Background] fetch port disconnected'); });
    }
  } catch (e) { console.warn('[Background] fetch port onConnect error', e); }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'FORWARD_OFFSCREEN') {
    ...
    const listener = (resp) => {
      try { port.onMessage.removeListener(listener); } catch (e) {}
      sendResponse(resp);
    };
    port.onMessage.addListener(listener);
    try { port.postMessage(payload); } catch (e) { try { port.onMessage.removeListener(listener); } catch (e2){} sendResponse({ ok: false, error: e.message }); }
    return true;
  }
