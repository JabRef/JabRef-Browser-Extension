  ...
  while ((m = fieldRe.exec(bibtex)) !== null) {
    ...
    if (key === "title") item.title = val;
      ...
      item.creators = authors.map((a) => {
        ...
        if (parts.length === 2)
          return { lastName: parts[0], firstName: parts[1], creatorType: "author" };
        const sp = a.split(" ").filter(Boolean);
        return {
          firstName: sp.slice(0, -1).join(" "),
          lastName: sp.slice(-1)[0],
          creatorType: "author",
        };
      });
    ...
    xpath: (d, xp) => {
      const resolver = {
        lookupNamespaceURI: (prefix) => {
          try {
            return (
              (doc.documentElement && doc.documentElement.lookupNamespaceURI(prefix)) ||
              (doc.lookupNamespaceURI && doc.lookupNamespaceURI(prefix)) ||
              null
            );
          } catch {
            return null;
                ...
                try {
                  ...
                  if (res && res.ok) {
                    ...
                    const item = {
                      ...
                      title: json.title || "",
                      volume: json.volume || "",
                      issue: json.issue || json["issue-number"] || "",
                      pages: json.page || json.pages || "",
                      date:
                        json.issued && json.issued["date-parts"] && json.issued["date-parts"][0]
                          ? String(json.issued["date-parts"][0][0])
                          : "",
                      ISSN: (json.ISSN && json.ISSN[0]) || "",
                      publicationTitle: json["container-title"] || "",
                      DOI: json.DOI || doi,
                    };
                    if (typeof handlers.itemDone === "function") handlers.itemDone(null, item);
                  }
                } catch {
                  /* ignore DOI resolution errors */
                }
              }
              if (typeof handlers.done === "function") handlers.done();
            } catch (err) {
              if (typeof handlers.error === "function") handlers.error(err);
            }
          },
        };
      }
  ...
  safeAssign(target, "requestDocument", async (u, opts) => {
    const txt = await (
      target.requestText
        ? target.requestText
        : async () => {
            throw new Error("requestText unavailable");
          }
    )(u, opts);
    return new DOMParser().parseFromString(txt, "text/html");
  });
