...
var CSL_TEXT_MAPPINGS = {
	...
	/* reporter and code should move to SQL mapping tables */
	"collection-title": ["seriesTitle", "series"],
	"collection-number": ["seriesNumber"],
	"publisher": ["publisher", "distributor"],
	/* distributor should move to SQL mapping tables */
	...
	/* these should be mapped to the same base field in SQL mapping tables */
	"event-place": ["place"],
...
Zotero.Utilities = {
	...
	cleanURL: function(url, tryHttp = false) {
		...
		try {
			...
			if (e instanceof Components.Exception && e.result == Components.results.NS_ERROR_MALFORMED_URI) {
	...
	"xpathText": function(node, xpath, namespaces, delimiter) {
		...
		for (var i = 0, n = elements.length; i < n; i++) {
			...
			strings[i] =
				(el.nodeType === 2 /*ATTRIBUTE_NODE*/ && "value" in el) ? el.value : "textContent" in el ? el.textContent : "innerText" in el ? el.innerText : "text" in el ? el.text : el.nodeValue;
		}
	...
	"varDump": function(obj, level, maxLevel, parentObjects, path) {
		...
		if (Zotero.isFx && !Zotero.isBookmarklet && (obj instanceof Components.interfaces.nsIDOMNode || obj instanceof Components.interfaces.nsIDOMWindow)) {
			level = maxLevel;
		}
			...
			for (var i = 0, n = fields.length; i < n; i++) {
				...
				if (field in zoteroItem) {
					...
					var fieldID = Zotero.ItemFields.getID(field),
						typeFieldID;
					if (fieldID && (typeFieldID = Zotero.ItemFields.getFieldIDFromTypeAndBase(itemTypeID, fieldID))) {
	...
	"itemFromCSLJSON": function(item, cslItem) {
		...
		if (cslItem.type == 'book') {
			...
			zoteroType = 'film';
			if (cslItem['collection-title'] || cslItem['publisher-place'] || cslItem['event-place'] || cslItem.volume || cslItem['number-of-volumes'] || cslItem.ISBN) {
			...
			if (variable in cslItem) {
				...
				if (Zotero.ItemFields.isValidForType(fieldID, itemTypeID)) {
					...
					if (cslDate.literal || cslDate.raw) {
						...
						if (cslDate["date-parts"] && typeof cslDate["date-parts"] === "object" && cslDate["date-parts"] !== null && typeof cslDate["date-parts"][0] === "object" && cslDate["date-parts"][0] !== null) {
