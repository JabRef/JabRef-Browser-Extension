// Taken from https://github.com/zotero/zotero-connectors/blob/ea060a0aa2fea1267049b5fc880e53aa6c915eeb/src/common/http.js
...
Zotero.HTTP = new (function () {
    ...
    try {
      ...
      if ((e.status === 429 || e.retryAfter) && options.backoff < options.maxBackoff) {
        if (e.status === 429) {
          ...
          Zotero.debug(
            `HTTP ${e.status} returned with Retry-After: ${e.retryAfter}, attempting a backoff #${options.backoff + 1}`,
          );
        }
        if (e.retryAfter && parseInt(e.retryAfter) == e.retryAfter) {
          if (e.retryAfter * 1000 > MAX_BACKOFF) {
            Zotero.debug(
              `HTTP 429 Retry-After ${e.retryAfter} is higher than max backoff. Not retrying.`,
            );
            throw e;
          }
  ...
  this._requestXHR = async function (method, url, options) {
    ...
    let sameOriginRequestViaSafari =
      Zotero.isSafari && Zotero.HTTP.isSameOrigin(url) && !options.headers["User-Agent"];
    ...
    if (["GET", "HEAD"].includes(method)) {
      ...
      if (options.headers["Content-Type"] !== "multipart/form-data") {
        ...
        logBody =
          `: ${options.body.substr(0, options.logBodyLength)}` + options.body.length >
    ...
    if (Zotero.isBackground) {
      ...
      let replaceHeaders = HEADERS_SPECIAL_HANDLING.filter(
    ...
    try {
      ...
      if (options.debug && (xmlhttp.responseType === "" || xmlhttp.responseType === "text")) {
        Zotero.debug(
          `HTTP ${xmlhttp.status} ${xmlhttp.responseURL} response: ${xmlhttp.responseText}`,
        );
      } else {
      ...
      let invalidDefaultStatus =
        options.successCodes === null && (xmlhttp.status < 200 || xmlhttp.status >= 300);
      let invalidStatus =
        Array.isArray(options.successCodes) && !options.successCodes.includes(xmlhttp.status);
      if (invalidDefaultStatus || invalidStatus) {
        // Firefox throws accessing responseText when the response is not text
        let responseText = ["", "text"].includes(xmlhttp.responseType)
          ? xmlhttp.responseText
          : undefined;
  ...
  this._requestFetch = async function (method, url, options) {
    ...
    try {
      ...
      let invalidDefaultStatus =
        options.successCodes === null && (response.status < 200 || response.status >= 300);
      let invalidStatus =
        Array.isArray(options.successCodes) && !options.successCodes.includes(response.status);
      if (invalidDefaultStatus || invalidStatus) {
        throw new Zotero.HTTP.StatusError(
          response,
          url,
  ...
  this.doGet = function (url, onDone, responseCharset, cookieSandbox, headers) {
    ...
    this.request("GET", url, { responseCharset, headers }).then(onDone, function (e) {
  ...
  this.doPost = function (url, body, onDone, headers, responseCharset) {
    ...
    this.request("POST", url, { body, responseCharset, headers }).then(onDone, function (e) {
      onDone({ status: e.status, responseText: e.responseText });
      throw e;
    });
    return true;
  ...
  this._attachHandlers = function (url, xmlhttp, options) {
    ...
    xmlhttp.onerror = xmlhttp.onabort = function () {
      var e = new Zotero.HTTP.StatusError(
        xmlhttp,
        url,
        ["", "text"].includes(xmlhttp.responseType) ? xmlhttp.responseText : undefined,
      );
      if (options.successCodes === false) {
