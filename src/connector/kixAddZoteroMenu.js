		...
		for (let mutation of mutations) {
			for (let node of mutation.addedNodes) {
				if (node.id == 'docs-menubar') {
					...
					node.insertBefore(templateElem.content.firstChild, docsHelpMenu);
					menuAdded = true;
				}
				if (node.id == 'docs-toolbar') {
					...
					if (Zotero.isBrowserExt) {
						...
						imageURL = safari.extension.baseURI + 'images/zotero-z-16px-offline.png';
					}
					...
					node.insertBefore(templateElem.content.firstChild, docsMoreButton);
					buttonAdded = true;
				}
			}
		}
