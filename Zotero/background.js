...
Zotero.Connector_Browser = new function() {
		...
		return new Zotero.Promise(function(resolve) {
			chrome.windows.get(tab.windowId, null, function(win) {
				...
				chrome.windows.create({
						url: chrome.extension.getURL("itemSelector/itemSelector.html") + "#" + encodeURIComponent(JSON.stringify([tab.id, items]))
	...
	function _showTranslatorIcon(tab, translator) {
		...
		chrome.browserAction.setIcon({
			tabId: tab.id,
			path: (itemType === "multiple" ? "images/treesource-collection.png" : Zotero.ItemTypes.getImageSrc(itemType))
		});
	...
	function _showPreferencesContextMenuItem() {
		...
		chrome.contextMenus.create({
			...
			onclick: function() {
				chrome.tabs.create({
					url: chrome.extension.getURL('preferences/preferences.html')
				});
			},
			contexts: ['all']
		});
