...
export function createZoteroShim() {
  const Zotero = {
    _lastItem: null,
    debug: () => {},
    // loadTranslator only supports 'import' translators in this minimal shim.
    loadTranslator: (type) => {
      ...
      return {
        ...
        translate: async () => {
          ...
          try {
            // If storedString looks like RIS, attempt to convert to BibTeX via ris.js
            let bib = storedString;
            if (storedString && /^TY  - /m.test(storedString)) {
              try {
                // dynamic import; callers should ensure ris.js is available relative to their context
                ...
                // ignore and keep storedString as-is
              }
            }

            // Convert BibTeX to an item-like object as a fallback, else make a minimal item
            ...
            // Invoke translator's itemDone handler (many translators call item.complete())
            try {
              itemHandler(null, item);
            } catch (e) {
              // Some translators mutate the item and call complete() asynchronously.
              // Swallow errors from the handler but continue to wait for complete().
              console.warn('itemDone handler threw', e);
            }

            // Wait for either complete() or a short timeout
